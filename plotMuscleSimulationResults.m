function plotMuscleSimulationResults()

    simData=readMotFile('massMuscle_states_degrees.mot');

    fig1 = figure('Name','Muscle Mechanics');
    
    h1=subplot(3,2,1);
    plot(simData.time, (0.5-simData.massToGround_yTranslation)/0.5, 'k');
    ylabel('Mass translation (% Resting length)');
    xlabel('Simulation time (s)');
    
    h2=subplot(3,2,2);
    plot(simData.time, (simData.massToGround_yTranslation), 'b');
    ylabel('Mass translation (m)');
    xlabel('Simulation time (s)');
    
    h3=subplot(3,2,3);
    plot(simData.time, (simData.massToGround_yTranslation_u), 'g');
    ylabel('Mass velocity (m/s)');
    xlabel('Simulation time (s)');
    
    h4=subplot(3,2,4);
    plot(simData.time, (simData.muscle_activation), 'p');
    ylabel('Muscle activation (0-1)');
    xlabel('Simulation time (s)');
    
    h5=subplot(3,2,5);
    plot(simData.time, (simData.muscle_fiber_length), 'c');
    ylabel('Muscle fibre length (0-1)');
    xlabel('Simulation time (s)');
    
end

function out = readMotFile(filename)
